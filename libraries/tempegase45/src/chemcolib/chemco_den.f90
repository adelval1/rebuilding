...
!      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
       kf = 1.0d0 * FRRC(j,1) * (T**(FRRC(j,2))) * exp(-FRRC(j,3)/T)
       kb = kf/kc(j)
      !kb = (kf+1.0d-50)/(kc(j)+1.0d-20)
!       write(*,*)'T :',T!pietro
!       write(*,*)'A,n,E : ',FRRC(j,1),FRRC(j,2),FRRC(j,3) !pietro
!       write(*,*)'inside chem, A : ',FRRC(j,1) !pietro
!       write(*,*)'inside chem, n : ',FRRC(j,2) !pietro
!       write(*,*)'inside chem, E : ',FRRC(j,3) !pietro
       
!       write(*,*) kf,kb,kc(j) !pietro
...
!      ^^^^^^^^^^^^^^^^
       ...
       DO i = 1, nsp
!       	  write(*,*)'kf*r_prod, kb*p_prod, STO(j,i) : ',kf*r_prod,kb*p_prod,STO(j,i)!pietro
	  !write(*,*)'kb*p_prod',kb*p_prod!pietro
	  !write(*,*)'STO(j,i)',STO(j,i)!pietro
          W(i) = W(i) + STO(j,i)*(kf*r_prod-kb*p_prod)!pietro 
       END DO
