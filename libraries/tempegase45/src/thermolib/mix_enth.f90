...
! //                                                                  //
! //  Output: enth(8)      array of  enthalpies                       //
...
!
      ...
      REAL(kind=8) T,TNEQ(4),enth(8),x(:),sent(8)
!
...
101           FORMAT (11x,'Routine: mixture_enthalpy. Value:',d15.8)
...
102   FORMAT(11x,'Routine: mixture_enthalpy. Index:',i3,' Value:',d15.8)
      ...
      END DO
!
!     Build enthalpy array by recursive calls to species enthalpies
!     ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
      DO i=1,nsp
          DO j=1,8
              sent(j)=0.0d0
          END DO
          CALL SPECIES_ENTHALPY (i,T,TNEQ,&
     &flg_anha,mode,flg_neq,flg_stop,flg_termo,sent)
          DO j=1,8
              enth(j)=enth(j)+sent(j)*x(i)
          END DO
      END DO
!
!     Including formation energy in the total for the mixture
!     ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
      enth(1)=enth(1)+enth(8)
!
!     Error handling
!     ^^^^^^^^^^^^^^
