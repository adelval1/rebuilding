...
! //                                                                  //
! //  Output: gibbs(8)      array of  gibbs free energies             //
! //              1           complete                                //
! //              2           translation                             //
! //              3           rotation (anha=0)                       //
! //              4           vibration (anha=0)                      //
...
!
      ...
      REAL(kind=8) var,T,TNEQ(4),gibbs(8),x(:),sent(8),varsent
!
...
101           FORMAT (11x,'Routine: mixture_gibbs. Value:',d15.8)
...
102   FORMAT (11x,'Routine: mixture_gibbs. Index:',i3,' Value:',d15.8)
...
103       FORMAT (11x,'Routine: mixture_gibbs. Value:',d15.8)
      ...
      END DO
!     ------------------------------------------------------------------
!     Build gibbs array by recursive calls to species energies
...
!     ------------------------------------------------------------------
      DO i=1,nsp
          ...
          IF (varsent.GT.1.0d-14) THEN
              CALL SPECIES_GIBBS (i,varsent,T,TNEQ,&
     &flg_anha,mode,flg_neq,flg_stop,flg_termo,sent)
          ENDIF
          DO j=1,8
              gibbs(j)=gibbs(j)+sent(j)*x(i)
          END DO
      END DO
!
!     Error handling
